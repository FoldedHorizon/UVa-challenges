...
uint32_t GetCycleLength(uint32_t i)
{
    uint32_t counter = 1;
    while (i != 1)
    {
...
uint32_t GetMaxCycle(uint32_t floor, uint32_t ceil)
{
    uint32_t out = 0;
    for (uint32_t i = floor; i <= ceil; i++)
    {
        //std::cout << i <<" | ";
        out = std::max(out, GetCycleLength(i));
        //std::cout << std::endl;
    }
    return out;
}
...
int main()
{
    ...
    {
        uint32_t result;
        result = i <= j ? GetMaxCycle(i,j) : GetMaxCycle(j,i);
